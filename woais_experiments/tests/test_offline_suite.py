...
class TestOfflineSuite(unittest.TestCase):
    def test_run_writes_results_and_preserves_hashes(self):
        ...
        self.assertTrue((RESULTS / "routing" / "oracle_budget_sweep.json").exists())
        self.assertTrue((RESULTS / "routing" / "oracle_budget_sweep.csv").exists())
        self.assertTrue((RESULTS / "accounting" / "breakeven" / "stage12.json").exists())
        self.assertTrue((RESULTS / "accounting" / "breakeven" / "stage12.csv").exists())
        self.assertTrue((RESULTS / "accounting" / "breakeven" / "stage12_usd.csv").exists())
        self.assertTrue((RESULTS / "accounting" / "breakeven" / "stage12_latency_ms.csv").exists())
        self.assertTrue((RESULTS / "accounting" / "breakeven" / "stage12_tokens.csv").exists())
        self.assertTrue((RESULTS / "routing" / "robustness.csv").exists())
        self.assertTrue((RESULTS / "routing" / "robustness.json").exists())
        self.assertTrue((RESULTS / "latency" / "stage12_wallclock.json").exists())
        self.assertTrue((RESULTS / "latency" / "frontier_per_query.csv").exists())
        self.assertTrue((RESULTS / "latency" / "frontier_aggregates.json").exists())
        self.assertTrue((RESULTS / "latency" / "serverless_model.json").exists())
        self.assertTrue((RESULTS / "external" / "routellm_tables.json").exists())
        self.assertTrue((RESULTS / "external" / "audit" / "index.json").exists())
        self.assertTrue((RESULTS / "external" / "audit" / "routellm_s9_committed.json").exists())
        self.assertTrue((RESULTS / "external" / "audit" / "ecologic_stage12.json").exists())
        self.assertTrue((RESULTS / "workloads" / "frozen.json").exists())
        self.assertTrue((RESULTS / "workloads" / "simulator_sweep.json").exists())
        self.assertTrue((RESULTS / "workloads" / "simulator_sweep.csv").exists())
