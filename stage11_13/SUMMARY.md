...
eight router families up to a prompted 70B model and a LoRA-fine-tuned 27B
model, and a learning curve whose power-law asymptote is 0.74 AUC.
...
|---|---|---|
| **11** | Formalise the decomposition; prove it; validate it; measure it on RouterBench | **Complete.** 14/14 propositions validated against brute-force LP/Monte-Carlo. H1 **supported**, H2 **supported**. |
| **12** | Router-free predictability ceiling from replicate generations | **Complete, and it refuted our own H3.** Reported as such. |
| **12b** | Learning curves — would more data close the gap? | **Complete. Exploratory, not pre-registered** (see `DEVIATIONS.md` D3). |
| **13** | Router-strength ladder: prompted LLM router, fine-tuned LLM router | **Complete.** Pre-registered criterion C1 **not met** — the plateau survives. |
| **Paper** | Workshop-length draft with proofs and appendix | **Complete.** `../paper/main.tex`, compiles with `pdflatex`. |

Additional spend for these stages: **$9.96** against a pre-registered gate of
**$25**, enforced in code. Project total **$58.05**, against the original $150
envelope. RouterBench, the decomposition, the learning curves and all validation
cost **nothing** — they analyse already-released outcomes.
...
| LoRA fine-tune, 3 epochs | Gemma-3-27B-it | see `s13_llm_router.json` |

**Pre-registered criterion C1** — an LLM router must beat Stage 7c's best
(0.6816) by ≥ 0.05 for "you tested a weak router" to become live again — is
**not met**: the best LLM rung is +0.029. The plateau survives.

The random forest is the diagnostic: training AUC 0.9999 (perfect memorisation)
and held-out AUC *below* logistic regression. Ample capacity, no generalisation.
That is the signature of a target that is not a smooth function of the input
representation, not of an inadequate model class.

**Cost.** Pre-registered gate $25, enforced in code (`s13_spend.json`; every
call books the provider's own reported usage and the run aborts at the gate).
Actuals: prompted zero-shot $0.58, prompted 4-shot $2.41, LoRA training $6.71,
fine-tuned inference — see the ledger. Projection in `prereg_stage11_13.md` §4.1
was ~$16; actual is under that.
...
| `DEVIATIONS.md` | Five deviations, including **D1 (H3 refuted)** and **D2 (a pre-registration statement that was wrong)** |
...
| `s11_routerbench.py` / `_0shot.json` / `_pairs_0shot.csv.gz` | The decomposition on RouterBench: 55 pairs × 8 families × 9 operating points |
...
| `s13_llm_router.py` / `.json` / `s13_spend.json` / `s13_state.json` | Prompted and fine-tuned LLM routers, with the enforced cost gate and full ledger |
| `s13_prompted_{0,4}shot.jsonl`, `s13_finetuned.jsonl` | Per-(item, tier) router scores, resumable and keyed |
